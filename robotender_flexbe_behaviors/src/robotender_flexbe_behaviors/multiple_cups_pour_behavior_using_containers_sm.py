...
'''
class multiplecupspourbehaviorusingcontainersSM(Behavior):
	'''
	...
	def __init__(self):
		super(multiplecupspourbehaviorusingcontainersSM, self).__init__()
		self.name = 'multiple cups pour behavior using containers'
	...
	def create(self):
		...
		_state_machine.userdata.pour_to_left = [4.610045297589599, 4.343199701639057, 1.419019181003809, 3.012844793851002, 1.4674078859041673, 4.845438377916176]
		...
		_state_machine.userdata.pour_to_center = [4.680331784865464, 4.315325726089742, 1.4461706409493849, 2.7535296027166787, 1.4171899888090882, 0.5029200288136196]
		...
		_sm_stuff_with_left_cup_0 = OperatableStateMachine(outcomes=['planning_failed', 'control_failed', 'failed', 'reached'], input_keys=['left_values', 'post_pour_to_center', 'pre_grab_left', 'joint_names', 'CLOSE', 'prep_pour_to_center', 'pour_to_center', 'OPEN'])
		...
		# x:30 y:365, x:130 y:365, x:230 y:365, x:1150 y:560
		_sm_stuff_1 = OperatableStateMachine(outcomes=['failed', 'planning_failed', 'control_failed', 'reached'], input_keys=['OPEN', 'CLOSE', 'center_values', 'prep_pour_to_left', 'pour_to_left', 'post_pour_to_left', 'back_off_center', 'joint_names'], output_keys=['joint_names'])
